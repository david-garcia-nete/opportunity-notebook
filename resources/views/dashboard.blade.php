<x-app-layout>
    ...
    <div class="bg-gray-50 py-10">
        <div class="mx-auto max-w-7xl space-y-8 px-4 sm:px-6 lg:px-8">
            ...
            <section data-testid="focus-opportunity-readiness" class="rounded-2xl border border-gray-100 bg-white p-6 shadow-sm">
                <div class="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                    <div>
                        <p class="text-sm font-semibold uppercase tracking-wide text-indigo-600">Focus Opportunity Readiness</p>
                        <h3 class="mt-1 text-lg font-semibold text-gray-900">Am I prepared to win my focus opportunities?</h3>
                        <p class="mt-1 text-sm text-gray-500">Low-readiness focus opportunities are high-interest opportunities that need preparation before pursuit.</p>
                    </div>
                    <a href="{{ route('portfolio-readiness') }}" class="inline-flex items-center justify-center rounded-md bg-white px-4 py-2 text-sm font-semibold text-gray-700 shadow-sm ring-1 ring-inset ring-gray-300 transition hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">
                        View Portfolio Readiness
                    </a>
                </div>

                @if ($currentFocusReadiness->isEmpty())
                    <div class="mt-5 rounded-xl bg-gray-50 p-4 text-sm text-gray-500 ring-1 ring-inset ring-gray-100">
                        Mark opportunities as focus opportunities to review readiness here.
                    </div>
                @else
                    <div class="mt-6 grid gap-4 lg:grid-cols-2">
                        @foreach ($currentFocusReadiness as $summary)
                            @php($opportunity = $summary['opportunity'])
                            @php($readiness = $summary['indicators'])
                            <article class="rounded-xl p-5 ring-1 ring-inset {{ $readiness['is_low_readiness'] ? 'bg-red-50 ring-red-100' : 'bg-gray-50 ring-gray-100' }}">
                                <div class="flex items-start justify-between gap-4">
                                    <div>
                                        <a href="{{ route('opportunities.show', $opportunity) }}" class="font-semibold text-indigo-700 hover:text-indigo-900">{{ $opportunity->title }}</a>
                                        <p class="mt-1 text-sm text-gray-600">{{ $opportunity->company ?? 'No company listed' }}</p>
                                    </div>
                                    <div class="text-right">
                                        <p class="text-2xl font-bold text-gray-900">{{ $readiness['score'] }}</p>
                                        <span class="mt-1 inline-flex rounded-full bg-white px-2.5 py-1 text-xs font-semibold text-gray-700 ring-1 ring-inset ring-gray-200">{{ $readiness['status'] }}</span>
                                    </div>
                                </div>
                                <dl class="mt-4 grid gap-3 text-sm sm:grid-cols-2">
                                    <div>
                                        <dt class="font-medium text-gray-500">Open Critical Gaps</dt>
                                        <dd class="mt-1 font-semibold text-gray-900">{{ $readiness['open_critical_gaps_count'] }}</dd>
                                    </div>
                                    <div>
                                        <dt class="font-medium text-gray-500">Open High Gaps</dt>
                                        <dd class="mt-1 font-semibold text-gray-900">{{ $readiness['open_high_gaps_count'] }}</dd>
                                    </div>
                                </dl>
                                @if ($readiness['is_low_readiness'])
                                    <p class="mt-4 rounded-lg bg-white p-3 text-sm font-semibold text-red-800 ring-1 ring-inset ring-red-100">Low readiness: prepare evidence or close priority gaps before heavier pursuit.</p>
                                @endif
                            </article>
                        @endforeach
                    </div>
